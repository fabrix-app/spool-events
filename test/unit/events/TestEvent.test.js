...
describe('Event', () => {
  ...
  it('should publish an event to onAutoTestEvent.test', done => {
    try {
      global.app.services.EventsService.publish('test', {test: 'test'}, {done: done})
    }
    catch (err) {
      done(err)
    }
  ...
  it('should publish an event to onAutoTestEvent.test2', done => {
    try {
      global.app.services.EventsService.publish('test2', {test: 'test2'}, {done: done})
    }
    catch (err) {
      done(err)
    }
  ...
  it('should publish an event to onTestEvent.test3', done => {
    try {
      global.app.services.EventsService.publish('test3', {test: 'test3'}, {done: done})
    }
    catch (err) {
      done(err)
    }
  ...
  it('should publish an event to onTestEvent.test4', done => {
    try {
      global.app.services.EventsService.publish('test4', {test: 'test4'}, {done: done})
    }
    catch (err) {
      done(err)
    }
  ...
  it('should not publish an event to onNotTestEvent.test5', done => {
    try {
      global.app.services.EventsService.publish('test5', {test: 'test5'}, {done: done})
      done()
    }
    catch (err) {
      done(err)
  ...
  it('should not publish an event to onNotTestEvent.test6', done => {
    try {
      global.app.services.EventsService.publish('test6', {test: 'test6'}, {done: done})
      done()
    }
    catch (err) {
      done(err)
